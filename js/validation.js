document.addEventListener("DOMContentLoaded", function () {
  ...
  function validateNumber() {
      ...
      if (value.startsWith("+375") && value.length < 13) {
          showError(numberInput, "Номер должен содержать 13 символов, если начинается с +375.");
          return false;
      } else if (value.startsWith("80") && value.length < 11) {
          showError(numberInput, "Номер должен содержать 11 символов, если начинается с 80.");
          return false;
      }
  ...
  function validateAddress() {
      let value = addressInput.value.trimStart();
